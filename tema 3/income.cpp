...
void changeValueInIncome(char *value)
{
    HANDLE handle;
    int fileExists = 1;
    char path[] = "C:\\Facultate\\CSSO\\Laboratoare\\Week3\\Reports\\Summary\\income.txt";
    ...
    else
    {
        ...
        else
        {
            ...
            CloseHandle(handle);
            writeInFileNewValue("C:\\Facultate\\CSSO\\Laboratoare\\Week3\\Reports\\Summary\\income.txt", finalSummary);
        }
        CloseHandle(handle);
    }
    if (fileExists == 0)
    {
        ...
        writeInFile("C:\\Facultate\\CSSO\\Laboratoare\\Week3\\Reports\\Summary\\income.txt", finalSummary);
    }
}
...
{
    HANDLE handle;
    char path[] = "D:\\facultate\\Anul 3\\Semestrul 1\\CSSO\\tema 3\\H3_input\\income\\";
    strcat(path, param);
    // printf("%s\n",path);
    handle = CreateFileA((LPCSTR)path, GENERIC_READ, NULL, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    if (handle == INVALID_HANDLE_VALUE)
    ...
    else
    {
        ...
        else
        {
            ...
            while (value)
            {
                ...
                sprintf(textToWriteInFile, "Am facut o incasare in valoare de %s in data de %s\n", value, param);
...
void writeLogs()
{
    ...
    HANDLE handle;
    handle = FindFirstFile("D:\\facultate\\Anul 3\\Semestrul 1\\CSSO\\tema 3\\H3_input\\income\\*", &data);
    if (handle == INVALID_HANDLE_VALUE)
    {
        printf("Eroare");
        FindClose(handle);
